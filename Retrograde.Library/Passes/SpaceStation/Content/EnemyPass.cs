...
public class EnemyPass : IGenPass
{
    ...
    public void RunPass(DungeonState state)
    {
        ...
        float densityMultiplier = 0.8f + (float)RandomProvider.Random.NextDouble() * 0.6f;
        ...
        if (spacedSpawns.Count > 0)
        {
            ...
            if (eligibleIndices.Count > 0 && RandomProvider.Random.Next(100) < 30)
            {
                minibossIndex = eligibleIndices[RandomProvider.Random.Next(eligibleIndices.Count)];
            }
        }
    ...
    private static int DetermineGroupSize(string districtType)
    {
        ...
        return RandomProvider.Random.Next(min, max + 1);
    }
    ...
    private static List<SpawnCandidate> ChooseCandidates(List<SpawnCandidate> candidates, int targetCount)
    {
        ...
        for (int i = 0; i < targetCount && pool.Count > 0; i++)
        {
            ...
            float roll = (float)(RandomProvider.Random.NextDouble() * totalWeight);
            float cumulative = 0f;
            int chosenIndex = 0;
    ...
    private readonly float _safeZone = 0.05f + (float)RandomProvider.Random.NextDouble() * 0.15f;
    ...
    private readonly float _waveFrequency = 2.5f + (float)RandomProvider.Random.NextDouble() * 1.5f; // 2.5-4 cycles
    private readonly float _waveAmplitude = 0.4f + (float)RandomProvider.Random.NextDouble() * 0.2f; // 40-60% variation
    private readonly float _wavePhase = (float)RandomProvider.Random.NextDouble() * (float)Math.PI; // Random start phase
