...
public class EnemyAlertPrimitiveCoveragePass : IGenPass
{
    ...
    public void RunPass(DungeonState state)
    {
        ...
        foreach (var section in sections)
        {
            ...
            string selectedOption = isBossRoom
                ? DefendActivatorId
                : AlertOptions[RandomProvider.Random.Next(AlertOptions.Length)];
    ...
    private static void Subdivide(RgAabb area, List<RgAabb> roomBounds, List<RgAabb> output, int depth)
    {
        ...
        float splitFraction = 0.4f + (float)RandomProvider.Random.NextDouble() * 0.2f;
        float splitPoint;
    ...
    {
        foreach (var acti in RetrogradeContext.Current.StarfieldMod.Activators)
        {
