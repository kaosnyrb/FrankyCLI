...
namespace Retrograde.Passes
{
    ...
    public class LockedLootRoomPass : IGenPass
    {
        ...
        private void CreateKey(DungeonState state)
        {
            ...
            Key templateKey = new Key(RetrogradeContext.Current.TargetMod);
            Key refkey = null;
            foreach (var key in RetrogradeContext.Current.StarfieldMod.Keys)
            {
                if (key != null)
                {
                    refkey = key.DeepCopy();
        ...
        private static bool IsDoorObject(PlacedObject po)
        {
            ...
            try
            {
                if (RetrogradeContext.Current.TargetMod.Doors.ContainsKey(po.Base.FormKey))
                    return true;

                if (RetrogradeContext.Current.StarfieldMod.Doors.ContainsKey(po.Base.FormKey))
                    return true;
            }
            catch
            {
        ...
        private void PlaceKey(DungeonState state, PlacedRoom keyRoom)
        {
            ...
            if (prefabCell != null)
            {
                ...
                if (markers.Count > 0)
                {
                    ...
                    var chosenMarker = markers[RandomProvider.Random.Next(markers.Count)];
